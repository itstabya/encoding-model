...
%
% arguments: pred_allmat      - cell array corresponding to a matrix of behavioral predictors, each term is a trial and contains a matrix where rows are timepoints and columns are behavioral predictors.
...
%            neural_act_mat   - cell array corresponding to a matrix of activity traces, each term is a trial and contains a matrix where rows are timepoints and columns are traces correspoding to different
...
for cellctr = 1:numcells
    ...
    if num_cont_inds == 0
        all_non_cont_pred_inds = 1:size(cur_pred_allmat_z{1},2);
    else
